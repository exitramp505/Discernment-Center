(async function(){
 ...
 list.innerHTML=data.map(r=>{const title = r.scores?.assessmentTitle || (r.scores?.assessmentType === 'isa_readiness' ? 'Ministry Readiness Inventory' : 'Character Qualities Assessment'); const unit=r.scores?.assessmentType==='isa_readiness'?'%':''; return `<div class="reportListItem"><div><strong>${title}</strong><p class="muted">${new Date(r.created_at).toLocaleString()} · ${r.state||''} · ${r.region||''} Region</p></div><div class="scoreBadge"><strong>${r.overall||r.scores?.overall||''}${unit}</strong><span>${r.overall_label||r.scores?.overallLabel||''}</span></div><a class="buttonLink secondary" href="report.html?id=${encodeURIComponent(r.id)}">View Report</a></div>`}).join('')
})();
